...
class DBOperation
{
	...
	public function addProduct($cid,$bid,$code,$pro_name,$price,$prices,$stock,$date){
		$pre_stmt = $this->con->prepare("INSERT INTO `products`
			(`cid`, `bid`,`code_id` ,`product_name`, `product_price`, `productsell`,
			 `product_stock`, `added_date`, `p_status`)
			 VALUES (?,?,?,?,?,?,?,?,?)");
		$status = 1;
		$pre_stmt->bind_param("iissddisi",$cid,$bid,$code,$pro_name,$price,$prices,$stock,$date,$status);
		$result = $pre_stmt->execute() or die($this->con->error);
		if ($result) {
