$(document).ready(function(){
	...
	$("body").delegate(".edit_product","click",function(){
		...
		$.ajax({
			...
			success : function(data){
				...
				$("#product_price_sell").val(data["productsell"]);
				$("#product_qty").val(data["product_stock"]);
				$("#products_qty").val(data["stock_shop"]);
			}
		})
	})

	//Update product stock store
	$('#update_shop').on('show.bs.modal', function(e) {
	var bookId = $(e.relatedTarget).data('book-id').id;
	var remain = $(e.relatedTarget).data('book-id').remain;
	var remains = $(e.relatedTarget).data('book-id').sremain;

	$(e.currentTarget).find('input[name="product_qty"]').val(bookId);
	$(e.currentTarget).find('input[name="remain"]').val(remain);
	$(e.currentTarget).find('input[name="psdatao"]').val(remains);
	});


	$("#update_store_stock").on("submit",function(){
		
		
		$.ajax({
			url : "./includes/process.php",
			method : "POST",
			data : $("#update_store_stock").serialize(),
			success : function(data){
	...
	//Update product stock store
	$('#updates').on('show.bs.modal', function(e) {
		var bookId = $(e.relatedTarget).data('book-id').id;
		var remain = $(e.relatedTarget).data('book-id').remain;
		var remains = $(e.relatedTarget).data('book-id').sremain;
		
		$(e.currentTarget).find('input[name="id"]').val(bookId);
		$(e.currentTarget).find('input[name="stock"]').val(remain);
		$(e.currentTarget).find('input[name="msdatao"]').val(remains);
		});

		$("#update_god_stock").on("submit",function(){
			$.ajax({
					url : "./includes/process.php",
					method : "POST",
					data : $("#update_god_stock").serialize(),
					success : function(data){
